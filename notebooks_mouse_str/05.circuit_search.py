...
# 3. Sibling null profiles: `notebook_validation/old_sibling_profiles_SI_Nov2023.npz`
...
#
# Sibling null circuit profiles (1000 siblings, mutability-weighted gene sets).
# Each sibling undergoes the same SA circuit search; profiles show the
# bias–connectivity trade-off for optimised sibling circuits.

# %%
# Load sibling profiles
sib_npz = np.load("../notebook_validation/old_sibling_profiles_SI_Nov2023.npz")
sib_meanbias = sib_npz['meanbias']
sib_meanSI = sib_npz['meanSI']
sib_topbias_sub = sib_npz['topbias_sub']
sib_limits = sib_npz['bias_limits']
print(f"Siblings: {sib_topbias_sub.shape[0]}, "
      f"{sib_topbias_sub.shape[2]} bias limit points, "
      f"bias range [{sib_limits.min():.3f}, {sib_limits.max():.3f}]")

# %%
# Sibling profiles vs ASD pareto front
...
ax.plot(sib_meanSI, sib_meanbias, marker='.', color='orange', lw=2,
        markersize=8, ls='-', label='Avg Sibling')
...
# Dynamic ylim
all_bias = np.concatenate([sib_meanbias, pareto_df['mean_bias'].values])
ax.set_ylim(max(0, np.nanmin(all_bias) - 0.02),
            np.nanmax(all_bias) + 0.02)

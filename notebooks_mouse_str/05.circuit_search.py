...
# mutability-weighted sibling gene sets. We compare two sources:
# - **Legacy**: 1000 siblings from Nov 2023 (old SA codebase, converted to NPZ)
# - **New**: Fresh 1000 siblings from the standalone pipeline with current parquet

# %%
# Load both sibling profile sources
legacy_path = "../notebook_validation/old_sibling_profiles_SI_Nov2023.npz"
new_path = "../results/CircuitSearch_Sibling_Summary/Mutability/size_46/batch_0/sibling_profiles.npz"

# Legacy
legacy = np.load(legacy_path)
leg_meanbias, leg_meanSI = legacy['meanbias'], legacy['meanSI']
leg_profiles, leg_limits = legacy['topbias_sub'], legacy['bias_limits']
print(f"Legacy: {leg_profiles.shape[0]} siblings, "
      f"{leg_profiles.shape[2]} bias limit points, "
      f"bias range [{leg_limits.min():.3f}, {leg_limits.max():.3f}]")

# New
new = np.load(new_path)
new_meanbias, new_meanSI = new['meanbias'], new['meanSI']
new_profiles, new_limits = new['topbias_sub'], new['bias_limits']
print(f"New:    {new_profiles.shape[0]} siblings, "
      f"{new_profiles.shape[2]} bias limit points, "
      f"bias range [{new_limits.min():.3f}, {new_limits.max():.3f}]")

# %%
# Comparison statistics
print(f"{'':20s} {'Legacy':>12s} {'New':>12s}")
print(f"{'-'*44}")
print(f"{'N siblings':20s} {leg_profiles.shape[0]:12d} {new_profiles.shape[0]:12d}")
print(f"{'N bias limits':20s} {leg_profiles.shape[2]:12d} {new_profiles.shape[2]:12d}")
print(f"{'Bias range min':20s} {leg_limits.min():12.3f} {new_limits.min():12.3f}")
print(f"{'Bias range max':20s} {leg_limits.max():12.3f} {new_limits.max():12.3f}")
print(f"{'Avg bias min':20s} {leg_meanbias.min():12.4f} {new_meanbias.min():12.4f}")
print(f"{'Avg bias max':20s} {leg_meanbias.max():12.4f} {new_meanbias.max():12.4f}")
print(f"{'Avg score min':20s} {leg_meanSI.min():12.4f} {new_meanSI.min():12.4f}")
print(f"{'Avg score max':20s} {leg_meanSI.max():12.4f} {new_meanSI.max():12.4f}")

# %%
# Side-by-side: Legacy vs New sibling profiles
fig, axes = plt.subplots(1, 2, dpi=240, figsize=(8.4, 4), sharey=True, sharex=True)

for ax, profiles, meanbias, meanSI, title, avg_color in [
    (axes[0], leg_profiles, leg_meanbias, leg_meanSI, "Legacy (Nov 2023)", "orange"),
    (axes[1], new_profiles, new_meanbias, new_meanSI, "New (Fresh Run)", "#1f77b4"),
]:
    # ASD pareto front
    ax.plot(pareto_df['circuit_score'], pareto_df['mean_bias'],
            marker='.', color='#542788', lw=2, markersize=6, ls='-', label='ASD')
    # Individual sibling profiles (gray)
    ax.plot(profiles[:, 1, :].T, profiles[:, 0, :].T,
            color='grey', markersize=1, lw=0.5, ls='-', alpha=0.05)
    # Average sibling
    ax.plot(meanSI, meanbias, marker='.', color=avg_color, lw=2,
            markersize=6, ls='-', label='Avg Sibling')
    ax.set_title(title, fontsize=12)
    ax.set_xlabel("Circuit Connectivity Score", fontsize=11)
    ax.grid(True, alpha=0.2)
    ax.legend(loc='lower left', frameon=False, fontsize=9)

axes[0].set_ylabel("Average Mutation Bias", fontsize=11)

# Dynamic ylim from all data
all_bias = np.concatenate([leg_meanbias, new_meanbias,
                           pareto_df['mean_bias'].values])
ymin = max(0, np.nanmin(all_bias) - 0.02)
ymax = np.nanmax(all_bias) + 0.02
for ax in axes:
...
# %%
# Overlay: both average sibling curves + ASD pareto
...
# Legacy average
ax.plot(leg_meanSI, leg_meanbias, marker='.', color='orange', lw=2,
        markersize=8, ls='-', label='Avg Sibling (Legacy)')

# New average
ax.plot(new_meanSI, new_meanbias, marker='.', color='#1f77b4', lw=2,
        markersize=8, ls='-', label='Avg Sibling (New)')
...
# %%
# Set active sibling source for downstream combined plot
sib_meanbias, sib_meanSI = new_meanbias, new_meanSI
sib_topbias_sub = new_profiles
...
ax.set_ylabel("Average Mutation Bias", fontsize=14)
ax.grid(True, alpha=0.2)
all_bias_combined = np.concatenate([sib_meanbias, pareto_df['mean_bias'].values])
ax.set_ylim(max(0, np.nanmin(all_bias_combined) - 0.02),
            np.nanmax(all_bias_combined) + 0.02)
plt.tight_layout()
plt.show()

...
#
# Caudoputamen maps to both Caudate and Putamen in the human Power atlas.
# To avoid double-counting, we merge Caudate+Putamen into a single
# "Caudate-Putamen" region throughout (CP-merged analysis).
...
# Buch et al. fMRI connectivity matrices (3 phenotypic dimensions + atypical)
...
# Cross-species mapping (confidence tiers: High, Medium-high, Medium-low, Low)
RegionMapping = pd.read_csv(os.path.join(BUCH_DIR, 'mouse_human_region_mapping_S14.csv'))
RegionMappingT1 = RegionMapping[RegionMapping["Confidence"] != "Low"]  # High + Medium-high + Medium-low
RegionMappingT2 = RegionMapping[RegionMapping["Confidence"].isin(["High", "Medium-high"])]  # High + Medium-high
print(f"Mappings: ALL={len(RegionMapping)} | T1={len(RegionMappingT1)} | T2={len(RegionMappingT2)}")

# %% [markdown]
# ## 2. Preprocessing
# Aggregate mouse structure biases to human regions, then merge Caudate+Putamen.
...
def mergeCP(df):
    """Merge Caudate+Putamen and SMA+Premotor into single regions."""
    df = df.copy()
...
# Aggregate bias (CP-merge applied after fMRI join in analyze_fmri_gencic_correlation)
bias_all = aggregate_bias_by_tier(gencic_data, RegionMapping)
bias_T1 = aggregate_bias_by_tier(gencic_data, RegionMappingT1)
bias_T2 = aggregate_bias_by_tier(gencic_data, RegionMappingT2)
print(f"Aggregated bias: ALL={len(bias_all)} | T1={len(bias_T1)} | T2={len(bias_T2)} human regions")
...
def analyze_fmri_gencic_correlation(fmri_matrix, bias_data, dataset_name, verbose=True):
    ...
    merged = mergeCP(merged)  # Merge Caudate+Putamen, SMA+Premotor for both bias & fMRI
    if verbose:
        print(f"\n{'='*80}\n{dataset_name}\n{'='*80}")
...
# %%
merged_viq, _ = analyze_fmri_gencic_correlation(Fig2A, bias_all, "Verbal IQ")
merged_sa, _ = analyze_fmri_gencic_correlation(Fig2B, bias_all, "Social Affect")
merged_rrb, _ = analyze_fmri_gencic_correlation(Fig2C, bias_all, "RRB")

# %%
plot_combined([merged_sa, merged_rrb, merged_viq], "ALL REGIONS: GENCIC Bias vs Combined fMRI")

# %% [markdown]
# ## 5. Tier 1 Analysis (High + Medium-high + Medium-low confidence)

# %%
merged_viq_T1, _ = analyze_fmri_gencic_correlation(Fig2A, bias_T1, "Verbal IQ (T1)")
merged_sa_T1, _ = analyze_fmri_gencic_correlation(Fig2B, bias_T1, "Social Affect (T1)")
merged_rrb_T1, _ = analyze_fmri_gencic_correlation(Fig2C, bias_T1, "RRB (T1)")

# %%
plot_combined([merged_sa_T1, merged_rrb_T1, merged_viq_T1], "TIER 1 (High/Med-high/Med-low): GENCIC vs fMRI")

# %% [markdown]
# ## 6. Tier 2 Analysis (High + Medium-high confidence)

# %%
merged_viq_T2, _ = analyze_fmri_gencic_correlation(Fig2A, bias_T2, "Verbal IQ (T2)")
merged_sa_T2, _ = analyze_fmri_gencic_correlation(Fig2B, bias_T2, "Social Affect (T2)")
merged_rrb_T2, _ = analyze_fmri_gencic_correlation(Fig2C, bias_T2, "RRB (T2)")

# %%
plot_combined([merged_sa_T2, merged_rrb_T2, merged_viq_T2], "TIER 2 (High/Med-high): GENCIC vs fMRI")

# %% [markdown]
# ## 7. Sibling-Based Permutation P-Values
...
# %%
# Load sibling null bias (213 structures x 10,000 sims)
...
def compute_sibling_perm_pvalues(tier_mapping, merged_sa, merged_rrb, merged_viq, sib_null, tier_name):
    """Compute sibling permutation p-values for a given tier (CP-merged)."""
    # Group structures by human region
    structs_in_null = tier_mapping[tier_mapping['Mouse_Structure'].isin(gencic_struct_set & set(sib_null.index))]
    region_groups = structs_in_null.groupby('Representative_Human_Group')['Mouse_Structure'].apply(list).to_dict()

    # Aggregate null bias to human regions (mean per region)
    null_regions = sorted(region_groups.keys())
    null_agg = np.zeros((len(null_regions), sib_null.shape[1]))
    for i, region in enumerate(null_regions):
        null_agg[i] = sib_null.loc[region_groups[region]].mean(axis=0).values
    null_agg_df = mergeCP(pd.DataFrame(null_agg, index=null_regions, columns=sib_null.columns))

    # Align with fMRI data
    common_regions = null_agg_df.index.intersection(merged_sa.index)
    X_null = null_agg_df.loc[common_regions].values
    Y_fmri = (merged_sa.loc[common_regions, 'Total_Strength_Abs'].values
              + merged_rrb.loc[common_regions, 'Total_Strength_Abs'].values
              + merged_viq.loc[common_regions, 'Total_Strength_Abs'].values)

    # Observed correlations
    X_obs = merged_sa.loc[common_regions, 'Mean_Bias'].values
    obs_r, obs_p = pearsonr(X_obs, Y_fmri)
    obs_rho, obs_rho_p = spearmanr(X_obs, Y_fmri)

    # Vectorized Pearson null
    n_sims = X_null.shape[1]
    Y_c = Y_fmri - Y_fmri.mean()
    Y_ss = np.sum(Y_c**2)
    X_c = X_null - X_null.mean(axis=0)[np.newaxis, :]
    X_ss = np.sum(X_c**2, axis=0)
    null_r = (Y_c @ X_c) / np.sqrt(X_ss * Y_ss)

    # Spearman null
    Y_ranks = rankdata(Y_fmri)
    null_rho = np.empty(n_sims)
    for i in range(n_sims):
        null_rho[i] = np.corrcoef(rankdata(X_null[:, i]), Y_ranks)[0, 1]

    # One-tailed permutation p-values (testing positive correlation)
    p_perm_r = (np.sum(null_r >= obs_r) + 1) / (n_sims + 1)
    p_perm_rho = (np.sum(null_rho >= obs_rho) + 1) / (n_sims + 1)

    print(f"{tier_name} Sibling Permutation P-values (N={len(common_regions)} regions, {n_sims} sims):")
    print(f"  Pearson:  r = {obs_r:.4f}, p = {obs_p:.4f}, p_perm = {p_perm_r:.4f}")
    print(f"  Spearman: rho = {obs_rho:.4f}, p = {obs_rho_p:.4f}, p_perm = {p_perm_rho:.4f}")
    return p_perm_r, p_perm_rho


# T1 permutation p-values
p_perm_pearson_T1, p_perm_spearman_T1 = compute_sibling_perm_pvalues(
    RegionMappingT1, merged_sa_T1, merged_rrb_T1, merged_viq_T1, sib_null, "T1")

# T2 permutation p-values
p_perm_pearson_T2, p_perm_spearman_T2 = compute_sibling_perm_pvalues(
    RegionMappingT2, merged_sa_T2, merged_rrb_T2, merged_viq_T2, sib_null, "T2")

# %% [markdown]
# ### T1 with permutation p-values (Supplementary Figure 14b — combined tiers)

# %%
plot_combined([merged_sa_T1, merged_rrb_T1, merged_viq_T1],
              "High + Medium-high + Medium-low confidence",
              p_perm_pearson=p_perm_pearson_T1, p_perm_spearman=p_perm_spearman_T1)

# %% [markdown]
# ### T2 with permutation p-values (Supplementary Figure 14b — high confidence)

# %%
plot_combined([merged_sa_T2, merged_rrb_T2, merged_viq_T2],
...
# %%
summary = []
for tier_name, dfs, p_r, p_rho in [
    ('ALL', (merged_sa, merged_rrb, merged_viq), None, None),
    ('T1', (merged_sa_T1, merged_rrb_T1, merged_viq_T1), p_perm_pearson_T1, p_perm_spearman_T1),
    ('T2', (merged_sa_T2, merged_rrb_T2, merged_viq_T2), p_perm_pearson_T2, p_perm_spearman_T2),
]:
    Xs = dfs[0]['Mean_Bias']
    ...
    summary.append({
        ...
        'Pearson_r': linres.rvalue, 'Pearson_p': linres.pvalue,
        'Spearman_rho': rho, 'Spearman_p': pval,
        'Perm_p_Pearson': p_r, 'Perm_p_Spearman': p_rho,
    })

summary_df = pd.DataFrame(summary)
print("Combined SA + RRB + VIQ Analysis (CP-merged)")
print(summary_df.round(4).to_string())

# %% [markdown]
# ## 9. Atypical Connectivity (Fig2D)

# %%
analyze_fmri_gencic_correlation(Fig2D, bias_all, "Atypical (ALL)")
analyze_fmri_gencic_correlation(Fig2D, bias_T1, "Atypical (T1)")
analyze_fmri_gencic_correlation(Fig2D, bias_T2, "Atypical (T2)")
...
comprehensive_table = mergeCP(comprehensive_table)  # Merge Caudate+Putamen, SMA+Premotor

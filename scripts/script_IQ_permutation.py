...
class script_bootstrapping_mutations:
    ...
    def run(self):
        ...
        Mutation_n_Phenotype_perm["IQ"] = np.random.permutation(
            Mutation_n_Phenotype_perm["IQ"].values)
        HighIQ = Mutation_n_Phenotype_perm[Mutation_n_Phenotype_perm["IQ"]
                                    > 70]
        LowIQ = Mutation_n_Phenotype_perm[Mutation_n_Phenotype_perm["IQ"]
                                   <= 70]
        #HighIQ_gene2MutN = Mut2GeneDF(HighIQ_muts, 0.347, 0.194)
        HighIQ_gene2MutN = Mut2GeneDF(HighIQ)
        HighIQ_Spec = AvgSTRZ_Weighted(ExpZscoreMat, HighIQ_gene2MutN,
                                       Method=1, csv_fil="../dat/Unionize_bias/Permutations/IQ_permut/HighIQ.bias.perm.{}.csv".format(self.idx))

        LowIQ_gene2MutN = Mut2GeneDF(LowIQ)
        LowIQ_Spec = AvgSTRZ_Weighted(ExpZscoreMat, LowIQ_gene2MutN,
                                       Method=1, csv_fil="../dat/Unionize_bias/Permutations/IQ_permut/LowIQ.bias.perm.{}.csv".format(self.idx))

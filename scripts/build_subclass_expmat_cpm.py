#!/usr/bin/env python3
"""
Build Subclass V2/V3 Expression Matrices from log2(CPM+1) h5ad files.

Reproduces the legacy Subclass_V2/V3_ExpMat.csv files by reading Allen Brain
Cell Atlas log2-transformed h5ad files and computing mean(log2(CPM+1)) per gene
per subclass, separately for V2 and V3 chemistry.

The log2 h5ad files contain pre-computed log2(CPM+1) values from the Allen
Institute. This script reads those directly and computes per-subclass means.

Pipeline:
  log2 h5ad files (WMB-10Xv2 + WMB-10Xv3) + cell metadata + ENSMUSG→Entrez
    → Subclass_V2_ExpMat.csv, Subclass_V3_ExpMat.csv

Usage:
  python scripts/build_subclass_expmat_cpm.py \
...
def _process_chunk(data_chunk, chunk_subclasses, sums, counts):
    """Process one chunk: accumulate values per subclass."""
    i = 0
    while i < len(chunk_subclasses):
        ...
        while j < len(chunk_subclasses) and chunk_subclasses[j] == sc:
            j += 1
        if sc not in sums:
            sums[sc] = data_chunk[i:j].sum(axis=0)
            counts[sc] = j - i
        else:
            sums[sc] += data_chunk[i:j].sum(axis=0)
            counts[sc] += j - i
...
def process_one_h5ad(h5ad_path, feature_name, subclass_barcode_map,
    ...
    """Process a single log2(CPM+1) h5ad file for subclass-level aggregation.
    ...
    for start in range(0, len(all_indices), chunk_size):
        ...
        # Data is already log2(CPM+1), just convert to float64
        data_chunk = chunk.astype(np.float64)
        _process_chunk(data_chunk, chunk_subclasses, sums, counts)
...
def process_version(version, h5ad_dir, subclass_barcode_map, n_genes,
    ...
    """Process all log2 h5ad files for one chemistry version."""
    h5ad_files = {}
    for fname in sorted(os.listdir(h5ad_dir)):
        if fname.endswith("-log2.h5ad"):
            feature = fname.replace("-log2.h5ad", "")
            h5ad_files[feature] = os.path.join(h5ad_dir, fname)
    print(f"  Found {len(h5ad_files)} {version} log2 h5ad files")
...
def main():
    parser = argparse.ArgumentParser(
        description="Build Subclass V2/V3 expression matrices from log2(CPM+1) h5ad files"
    )
    parser.add_argument(
        "--h5ad-base",
    ...
    # Get gene info from a V3 log2 file
    ...
    for version, sums, counts, label in [
        ("V3", v3_sums, v3_counts, "Subclass_V3_ExpMat_CPM"),
        ("V2", v2_sums, v2_counts, "Subclass_V2_ExpMat_CPM"),
    ]:
        ...
        for subclass in sorted(sums.keys()):
            ...
            if count == 0:
                continue
            mean_vals = sums[subclass][col_indices] / count
            results[subclass] = pd.Series(mean_vals, index=entrez_ids, dtype=np.float64)

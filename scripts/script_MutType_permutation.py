...
class script_bootstrapping_mutations:
    ...
    def run(self):
        ...
        Mutation_n_Phenotype = Mutation_n_Phenotype[Mutation_n_Phenotype["IQ"]>70]

        Mutation_n_Phenotype_perm = Mutation_n_Phenotype.copy(deep=True)
        Mutation_n_Phenotype_perm["GeneEff"] = np.random.permutation(
            Mutation_n_Phenotype_perm["GeneEff"].values)
        LGD = Mutation_n_Phenotype_perm[Mutation_n_Phenotype_perm["GeneEff"]
                                    != "missense"]
        Dmis = Mutation_n_Phenotype_perm[Mutation_n_Phenotype_perm["GeneEff"]
                                   == "missense"]
        #HighIQ_gene2MutN = Mut2GeneDF(HighIQ_muts, 0.347, 0.194)
        LGD_gene2MutN = Mut2GeneDF(LGD)
        LGD_Spec = AvgSTRZ_Weighted(ExpZscoreMat, LGD_gene2MutN,
                                       Method=1, csv_fil="../dat/Unionize_bias/Permutations/HighIQ_MutType_permut/LGD.bias.perm.{}.csv".format(self.idx))

        Dmis_gene2MutN = Mut2GeneDF(Dmis)
        Dmis_Spec = AvgSTRZ_Weighted(ExpZscoreMat, Dmis_gene2MutN,
                                       Method=1, csv_fil="../dat/Unionize_bias/Permutations/HighIQ_MutType_permut/Dmis.bias.perm.{}.csv".format(self.idx))
